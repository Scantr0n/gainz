...
| 1.3 *(stretch)* | AI reasoning for injury substitutions | Upgrade | ✅ **Done 7/8** — the "Injured? Find substitutes" panel on Plan now calls Claude for a per-exercise reason + tailored alternatives, constrained to the app's actual exercise catalog (prevents hallucinated exercise IDs). Same demo-mode/error-fallback pattern as the split recommender — old static `INJURY_SWAPS` map kept as the fallback, never a dead end. Added a "not medical advice" disclaimer since this is the app's one spot closest to physical-safety guidance. Verified live: demo-mode badge, bad-key fallback with visible warning, deselect-clears-instantly. |
